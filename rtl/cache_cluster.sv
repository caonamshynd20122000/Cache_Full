...
// ============================================================================
// File Name   : cache_cluster.sv
// Module Name : cache_cluster
// Description : Multi-Unit Cache Cluster Subsystem.
//               Instantiates and arbitrates multiple cache_wrap instances 
//               across core and memory interfaces.
// ============================================================================

`include "cache_define.vh"

module cache_cluster import cache_pkg::*; #(
...
`endif
    mem_bus_if.slave     core_bus_if [NUM_INPUTS * NUM_REQS],
    mem_bus_if.master    mem_bus_if [MEM_PORTS]
);
...
`endif
    ...
    for (genvar i = 0; i < NUM_REQS; ++i) begin : g_core_arb
        ...
        for (genvar j = 0; j < NUM_INPUTS; ++j) begin : g_core_bus_tmp_if
            `ASSIGN_MEM_BUS_IF (core_bus_tmp_if[j], core_bus_if[j * NUM_REQS + i]);
        end
        ...
        for (genvar k = 0; k < NUM_CACHES; ++k) begin : g_arb_core_bus_if
            `ASSIGN_MEM_BUS_IF (arb_core_bus_if[k * NUM_REQS + i], arb_core_bus_tmp_if[k]);
        end
    end
    ...
    for (genvar i = 0; i < MEM_PORTS; ++i) begin : g_mem_bus_if
        ...
        for (genvar j = 0; j < NUM_CACHES; ++j) begin : g_arb_core_bus_tmp_if
            `ASSIGN_MEM_BUS_IF (arb_core_bus_tmp_if[j], cache_mem_bus_if[j * MEM_PORTS + i]);
        end
        ...
        if (WRITE_ENABLE) begin : g_we
            `ASSIGN_MEM_BUS_IF (mem_bus_if[i], mem_bus_tmp_if[0]);
        end else begin : g_ro
            `ASSIGN_MEM_BUS_RO_IF (mem_bus_if[i], mem_bus_tmp_if[0]);
        end
    end
