...
// ============================================================================
// File Name   : cache_bank.sv
// Module Name : cache_bank
// Description : Parameterized N-Way Cache Bank Pipeline Subsystem.
//               Full implementation with MSHR, pipeline stages, fill forwarding,
//               writeback sequencer, and atomic memory operations.
// ============================================================================

`include "cache_define.vh"

module cache_bank import cache_pkg::*; #(
...
`endif
    ...
    if (AMO_ENABLE) begin : g_amo
        ...
        cache_amo #(
            .IS_LLC          (IS_LLC),
            .NUM_RES_ENTRIES (`CFG_AMO_RS_SIZE),
    ...
    if (WRITE_ENABLE) begin : g_mreq_queue
        if (WRITEBACK) begin : g_wb
            ...
            assign mreq_queue_data   = wb_data_sec;
...
`ifdef DBG_TRACE_CACHE
    ...
    wire [`CFG_XLEN-1:0] mem_rsp_full_addr = `CS_BANK_TO_FULL_ADDR(mem_rsp_addr, BANK_ID);
    wire [`CFG_XLEN-1:0] replay_full_addr = `CS_BANK_TO_FULL_ADDR(replay_addr, BANK_ID);
    wire [`CFG_XLEN-1:0] core_req_full_addr = `CS_BANK_TO_FULL_ADDR(core_req_addr, BANK_ID);
    wire [`CFG_XLEN-1:0] full_addr_st0 = `CS_BANK_TO_FULL_ADDR(st0.req.addr, BANK_ID);
    wire [`CFG_XLEN-1:0] full_addr_st1 = `CS_BANK_TO_FULL_ADDR(st1.req.addr, BANK_ID);
    wire [`CFG_XLEN-1:0] mreq_queue_full_addr = `CS_BANK_TO_FULL_ADDR(mreq_queue_addr, BANK_ID);
