...
// ============================================================================
// File Name   : cache_wrap.sv
// Module Name : cache_wrap
// Description : Top-Level Cache Wrapper with Non-Cacheable Bypass & Passthrough.
//               Exact line-by-line conversion from open-source GPGPU cache wrap.
// ============================================================================

`include "cache_define.vh"

module cache_wrap import cache_pkg::*; #(
...
`endif
    mem_bus_if.slave     core_bus_if [NUM_REQS],
    ...
    if (BYPASS_ENABLE) begin : g_bypass
        ...
        for (genvar i = 0; i < NUM_REQS; ++i) begin : g_core_bus_cache_if
            `ASSIGN_MEM_BUS_IF (core_bus_cache_if[i], core_bus_if[i]);
        end
        for (genvar i = 0; i < MEM_PORTS; ++i) begin : g_mem_bus_tmp_if
            `ASSIGN_MEM_BUS_IF (mem_bus_tmp_if[i], mem_bus_cache_if[i]);
        end
    end

    for (genvar i = 0; i < MEM_PORTS; ++i) begin : g_mem_bus_if
        if (WRITE_ENABLE) begin : g_we
            `ASSIGN_MEM_BUS_IF (mem_bus_if[i], mem_bus_tmp_if[i]);
        end else begin : g_ro
            `ASSIGN_MEM_BUS_RO_IF (mem_bus_if[i], mem_bus_tmp_if[i]);
        end
    end
    ...
    if (PASSTHRU == 0) begin : g_cache
        ...
        for (genvar i = 0; i < NUM_REQS; ++i) begin : g_core_bus_cache_if
            `UNUSED_MEM_BUS_IF (core_bus_cache_if[i])
        end
        for (genvar i = 0; i < MEM_PORTS; ++i) begin : g_mem_bus_cache_if
            `INIT_MEM_BUS_IF (mem_bus_cache_if[i])
        end
    `ifdef PERF_ENABLE
