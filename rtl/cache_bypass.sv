...
// ============================================================================
// File Name   : cache_bypass.sv
// Module Name : cache_bypass
// Description : Non-Cacheable and Direct Passthrough Bus Switch & Arbiter.
//               Exact line-by-line conversion from open-source GPGPU cache bypass.
// ============================================================================

`include "cache_define.vh"

module cache_bypass import gpu_pkg::*; #(
    ...
    mem_bus_if.slave     core_bus_in_if [NUM_REQS],
    mem_bus_if.master    core_bus_out_if [NUM_REQS],
    mem_bus_if.slave     mem_bus_in_if [MEM_PORTS],
    mem_bus_if.master    mem_bus_out_if [MEM_PORTS]
);
    ...
    `STATIC_ASSERT(0 == (`MEM_IO_BASE_ADDR % `CFG_MEM_BLOCK_SIZE), ("invalid parameter"))
    ...
    for (genvar i = 0; i < NUM_REQS; ++i) begin : g_core_bus_nc_switch_if
        ...
        if (CACHE_ENABLE) begin : g_cache
            ...
            `INIT_MEM_BUS_IF (core_bus_out_if[i])
        end
    end
    ...
    for (genvar i = 0; i < MEM_PORTS; ++i) begin : g_mem_bus_out_src
        `ASSIGN_MEM_BUS_IF_EX(mem_bus_out_src_if[0 * MEM_PORTS + i], mem_bus_out_nc_if[i], MEM_TAG_OUT_WIDTH, MEM_TAG_NC2_WIDTH, UUID_WIDTH);
        if (CACHE_ENABLE) begin : g_cache
            `ASSIGN_MEM_BUS_IF_EX(mem_bus_out_src_if[1 * MEM_PORTS + i], mem_bus_in_if[i], MEM_TAG_OUT_WIDTH, MEM_TAG_IN_WIDTH, UUID_WIDTH);
        end else begin : g_no_cache
            `UNUSED_MEM_BUS_IF(mem_bus_in_if[i])
        end
    end
